...
pub trait OdbBackend: 'static {
    ...
    /// This method is provided for compatibility
    /// with custom backends which are not able to support streaming writes
    fn write(&self, oid: &Oid, buffer: &[u8], object_type: git_object_t) -> Result<(), Error>;
...
pub extern "C" fn _git_dyn_odbbackend_read(
    ...
    unsafe {
        ...
        *output_buffer_size = read.data.len() as size_t;
...
pub extern "C" fn _git_dyn_odbbackend_read_prefix(
    ...
    unsafe {
        ...
        let odb_backend_ref: &mut OdbBackendSneakstructure = (backend
            as *mut OdbBackendSneakstructure)
            .as_mut()
            .expect("GIT-ODB-Backend read_prefix should never be called with a null pointer");
        ...
        *obj_buffer_len = read.1.data.len() as size_t;
...
/// # returns
/// `0` or an error code
pub extern "C" fn _git_dyn_odbbackend_read_header(
    len_out: *mut size_t,
    type_out: *mut git_object_t,
    backend: *mut git_odb_backend,
    obj_id: *const git_oid,
) -> c_int {
    unsafe {
        // Obtain Reference
        let odb_backend_ref: &mut OdbBackendSneakstructure = (backend
            as *mut OdbBackendSneakstructure)
            .as_mut()
            .expect("GIT-ODB-Backend read_header should never be called with a null pointer");

        // Call Trait Method
        let object_info = match odb_backend_ref
            .rust_impl
            .read_header(&Oid::from_raw(obj_id))
        {
        ...
        // Convert Results
        *len_out = object_info.size;
        *type_out = object_info.git_object_type;
...
/// # returns
/// `0` or an error code
pub extern "C" fn _git_dyn_odbbackend_write(
    backend: *mut git_odb_backend,
    id: *const git_oid,
    data: *const c_void,
    len: size_t,
    ...
    unsafe {
        ...
        let odb_backend_ref: &mut OdbBackendSneakstructure = (backend
            as *mut OdbBackendSneakstructure)
            .as_mut()
            .expect("GIT-ODB-Backend write should never be called with a null pointer");

        let buffer_slice: &[u8] = slice_from_raw_parts(std::mem::transmute(data), len)
            .as_ref()
            .expect("Reference must not be null");

        // Call Trait Method
        match odb_backend_ref
            .rust_impl
            .write(&Oid::from_raw(id), buffer_slice, r#type)
        {
            Ok(()) => (),
            Err(git_err) => {
                let err_code = git_err.raw_set_git_error();
